let Population = {
    ...
    render: function () {
        d3.json(Population.url)
            .then(function (jsonData) {
                Population.renderChart1(jsonData);
                Population.renderChart2(jsonData, 2001);
                Population.renderChart3(jsonData, 2001);
            });
    },
    renderChart1: function (jsonData) {
        ...
        for (let i = yearDomain[0]; i < yearDomain[1]; i++) {
            ...
            for (let j = 0; j < 3; j++) {
                sums[0] += jsonData[i]["總計"]["total"]["total"][j];
            }
            for (let j = 4; j < 13; j++) {
                sums[1] += jsonData[i]["總計"]["total"]["total"][j];
            }
            for (let j = 13; j < jsonData[i]["總計"]["total"]["total"].length; j++) {
                sums[2] += jsonData[i]["總計"]["total"]["total"][j];
            }
    ...
    renderChart2: function (jsonData, year) {
            ...
            svg.append("text")
                ...
                .on("mouseover", function (event, d) {
                    if (d3.select(this).style("stroke") == "none") {
                        Population.renderChart2(jsonData, i);
                    }
                });
        }
